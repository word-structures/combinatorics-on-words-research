...
## Adversarial mechanism and novelty discipline (mandatory whenever an explanation or a novelty claim is being formed)

*The claims protocol governs whether a statement is **true**. The artifact protocol governs whether the delivered object is **correct**. This section strengthens two previously under-governed axes: **mechanism/explanation**, which had no dedicated status or adversarial demotion path, and **novelty**, which already had Gate 4 but lacked explicit AI-provenance uncertainty, adversarial equivalence/subsumption search, and residual-risk status. Both are places where a fluent agent can assemble a persuasive story faster than the mathematics can justify one. Conceptual home: `EPISTEMIC_DISCIPLINE.md` §13 (axis separation), §14 (object identity and closure), §5 (independence). Gate integration: `PAPER_LIFECYCLE.md` Gates 0, 3 and 4.*

1. **THE AXES ARE SEPARATE:** truth, mechanism, generality, importance and novelty are five different questions, and standing on one is never evidence for another. An exact computation is not an explanation; a mechanism surviving twice is not a theorem; a theorem is not new; a new result is not important; and an attractive interpretation is not true. Each axis has exactly one home — see the table in `EPISTEMIC_DISCIPLINE.md` §13.

2. **FALSIFIER BEFORE SUPPORTING STORY:** before a mechanism is used to steer research, name a paper, or explain a phenomenon, write at least one concrete kill criterion — *before* running the experiments whose purpose is to support it. The order is `observation → falsifiable hypothesis → written kill criterion → kill test → interpretation → cross-instance prediction → proof attempt`, never `observation → attractive interpretation → supporting tests → moved goalposts`. A falsifier must be concrete enough to fail: *"see whether it still looks plausible"* is not one; *"if the descriptor is sufficient, two states sharing it must have identical response types — one counterexample kills sufficiency"* is. **This does not apply to exploratory notes.** It applies from the moment a mechanism is steering something.

3. **MATCHED CONTROLS BEFORE A STRUCTURAL INTERPRETATION:** when a rank, dimension, observability, recovery, compression, partition, spectral or other linear-algebraic phenomenon is interpreted as *special structure*, first test whether a matched generic control reproduces it — a random partition with the same family-size distribution, matched dimensions, a scrambled descriptor, or an alternative natural descriptor. *"A structured object has property X"* does not license *"the structure causes X"* when matched controls also have X. Controls are matched to the claim being tested, not run universally: where genericity is not a plausible competing explanation, write down why instead.

4. **A MECHANISM IS DEMOTED WITHOUT DEMOTING THE FACT:** mechanism status is tracked in `MATH_CLAIMS.md`'s own mechanism column, orthogonal to the verification status. When a kill test succeeds, the exact fact keeps its status, the mechanism is marked `FALSIFIED`, and the negative result is written to `NEGATIVE_RESULTS.md` with its scope. *The fact survived; the mechanism did not* is a normal and reportable outcome. Do not replace a failed mechanism with a nearby story and present the sequence as uninterrupted progress.

5. **NAME THE OBJECT BEFORE INTERPRETING ITS NUMBER, AND CHECK CLOSURE:** every decision-gating number carries a mathematical noun — which matrix, restricted to which state selection. Raw and quotient states, recurrent cores and recurrent-class members, induced subgraphs and closed subsystems, startup and saturated regimes, full and persistent futures are different objects and never share an informal label. If a restricted subset is used as a standalone counting or dynamical system, verify `x ∈ S ∧ x → y legal ⇒ y ∈ S`; where closure fails the induced system answers a different question while still returning plausible numbers. Full rule: `EPISTEMIC_DISCIPLINE.md` §14.

6. **AI MAY PROPOSE NOVELTY; AI MAY NOT CERTIFY ITS OWN NOVELTY:** every potentially novel result originating in this workflow starts at `NOVELTY_STATUS = NOT_ESTABLISHED` and stays there until an adversarial prior-art review is documented in `LITERATURE_COVERAGE.md`. Model confidence is not novelty evidence; absence of a citation is not novelty evidence; a model failing to recall a source is not novelty evidence; several models agreeing that something is new is not evidence of independent provenance. A mathematically correct theorem may be a rediscovery, and a derivation produced in the current session does not establish that the result was absent from prior literature or from training data. This is a problem of **unrecoverable provenance, not of deception** — say *"the model may reconstruct prior structure without recoverable provenance"*, never *"the model copied this and hid the source"*.

7. **AI PLURALITY IS NOT INDEPENDENCE:** a second AI analysis is corroboration, not an independent verification channel, and neither is another session, another prompt, or another vendor whose training and sources may be correlated. Independence is reported as the list of axes that actually differ (`EPISTEMIC_DISCIPLINE.md` §5), and at least one non-model axis must differ before a check counts as independent. This project has already miscounted once: `MATH_CLAIMS.md` row 6c described a "fourfold verification" of which one channel was another AI reading the same artifacts.
