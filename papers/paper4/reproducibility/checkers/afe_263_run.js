...
const X = require('../lib/afe_only_crosscheck.js');
const G = require('../lib/gate.js');
...
const hM = JSON.parse(fs.readFileSync(__dirname + '/../runs/h_matched_quota.json', 'utf8'));
const bcd = jl(__dirname + '/../runs/bcdBIG_H/pairs.jsonl');
...
for (let i = 0; i < pairs.length; i++) {
  ...
  if (!match) {
    ...
    fs.writeFileSync(__dirname + '/../runs/afe_263_MISMATCH.json', JSON.stringify({ pair: p, result: r, rows }, null, 1));
    process.exit(4);
  }
}
...
fs.writeFileSync(__dirname + '/../runs/afe_263_crosscheck.json', JSON.stringify({ summary, negativeControls: ctlOut, rows }, null, 1));
...
fs.writeFileSync(__dirname + '/../runs/AFE_EXISTS_263_CROSSCHECK_RESULTS_2026-08-29.csv', csv.join('\n') + '\n');
