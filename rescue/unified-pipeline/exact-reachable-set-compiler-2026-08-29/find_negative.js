...
for(let attempt = 0; attempt < 5000; attempt++) {
    ...
    for (let i=0; i<9; i++) {
        let b = "";
        for(let j=0; j<L; j++) b += Math.floor(Math.random()*3);
        concreteBlocks.push(b);
    }
    
    let sourceRoles = [];
    let chars = ['a','b','c','d','e'];
    for(let i=0; i<9; i++) sourceRoles.push(chars[Math.floor(Math.random()*2)]); // mostly a, b
    ...
    if (!rs.includes(minusTStr)) continue; // Not in Danger Zone
    
    // In Danger Zone! Check if square exists
    ...
    if (!anySquareExists) {
        console.log(`FOUND NEGATIVE CONTROL!`);
        console.log(`u=${u}, K=${K}, roles=${sourceRoles.join('')}`);
        break;
    }
}
