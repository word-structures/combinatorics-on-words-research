...
function computeClosure(B) {
    ...
    let head = 0;
    while(head < Q.length) {
        if (Q.length > 50) return null; // Too many states, abort
        let state = Q[head++];
...
console.log("Testing closures for sB...");
let s = "abacabadc";
...
while(queue.length > 0) {
    ...
    if (B_str.length > 3) continue; // limit length to 3 first
    ...
    if (Q !== null) {
        ...
        // Now check if W1 avoids Q
        // (Implementation of checking if W1 avoids Q goes here)
        break;
    }
    for(let a of alphabet) queue.push(B_str + a);
}
