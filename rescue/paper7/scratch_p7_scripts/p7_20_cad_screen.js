...
for (let m = M_START; m <= M_END; m++) {
    ...
    console.log(`m=${m} wm=${wm.slice(0, 10)}... profile=${profile.join(',')}`);
    candidates.push({ m, profile, finalSize: frontier.length });
}
candidates.sort((a,b) => a.finalSize - b.finalSize);
console.log("TOP 3 PROMOTED:");
for(let i=0; i<3; i++) console.log(candidates[i]);
