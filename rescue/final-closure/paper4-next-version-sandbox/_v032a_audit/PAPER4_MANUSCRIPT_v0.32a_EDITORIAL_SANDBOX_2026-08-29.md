...
**Working manuscript v0.32 — 2026-08-29**  
**Status:** sandbox integrated draft; not canonical; not submission-ready.  
**Editorial base:** sandbox v0.31, itself derived from `PAPER4_MANUSCRIPT_v0.30_2026-08-27.md`.  
...
The use of the fixed factor language is essential. A universal morphism from six source letters to three target letters cannot preserve Abelian-square-freeness on all Abelian-square-free source words: Carpi's necessary rank condition for the relevant preservation problem forces a commutatively bijective incidence structure, which is impossible for six image-Parikh vectors in \(\mathbb Z^3\). The present program therefore relies on the restricted, nonperiodic macro language rather than on universal preservation.
...
### Theorem 5.1 — finite subset-factor gate
...
The morphic or recurrent nature of \(x\) is not needed for the theorem; it is only one way to determine the finite factor language.
...
9,\ 15,\ 19,\ 19,\ 19,\ 19
\]
for \(L=2,3,4,5,6,7\), respectively. At \(L=4\) the count has reached \(19\), but \(Z_s\) is still empty. The first genuine same-block \(K\ge2\) configuration occurs at \(L=5\), so \(L\ge5\) is the natural stable statement.
...
X_2(u)=X_2(v)=(1,1).
\]
Append the **same** future letter \(a\) to both prefixes, and consider a future constraint that still references the historical depth \(1\):
\[
X_1-2X_2+X_3=0.
\]
For \(aba\),
...
X_1-2X_2+X_3=(0,0).
\]
Thus identical current states \(X_2\) can have different future legality under an identical continuation whenever an unclosed constraint still remembers an earlier prefix state. Therefore \(X_2\) alone does not determine the continuation language.
...
### Proposition 12.4 — translation-invariant pure-square mass
...
263/263
\]
quota-matched canonical H pairs.
...
## 15. Search architecture after the structural revision

The previous manuscript centered on large finite \(K\le40\) scaffold populations. Those records remain valid within their declared gates, but they are no longer the preferred route to a complete construction: a partial assignment should first be made complete for every square supported by the assigned role subset.
...
The immediate mathematical/computational questions are now:

1. characterize AFE-positive versus AFE-negative fixed-profile path systems using certificate-level invariants rather than ad hoc DFS statistics;
2. use the complete subset covers at every subsequent role stage;
3. preserve target/support separation so that the six-domain compiler can be reused instead of re-enumerating raw windows;
4. if a complete six-role \(H\) is found, freeze the six blocks immediately and run two independent finite-gate checks followed by the long-period certificate.
...
(+1,-2,+1)
\]
prefix algebra, Euclidean carries, mechanical words, finite-state paths with Parikh counters, or template sieving.
...
The six-domain/19-family theorem and the **Phase-I** AFE cutset core (path equivalence, frontier sufficiency, first-hit antichain/cylinder partition, and full-trie certificate) have each received a separate clean-room reconstruction. The newer weighted frontier-DAG Phase-II theorem remains independently unaudited.

### 17.2 Exact finite computations
...
- two-solver \(263/263\) agreement on the quota-matched H population;
- earlier exact finite block-search ledgers retained in the project archive.

### 17.3 Not established
...
- J. D. Currie and N. Rampersad, *Fixed points avoiding Abelian \(k\)-powers*, Journal of Combinatorial Theory, Series A 119 (2012), 942--948.
- S. Eyidoğan, H. Göral and N. Tanısalı, *Box Progressions, Abelian Power-Free Morphisms and A Sieve Technique for the Template Method*, Mathematics of Computation (2026), DOI 10.1090/mcom/4246.
...
- V. Keränen, *Combinatorics on Words — Suppression of Unfavorable Factors in Pattern Avoidance*, The Mathematica Journal 11 (2010).
- M. Rao and M. Rosenfeld, *Avoiding Two Consecutive Blocks of Same Size and Same Sum over \(\mathbb Z^2\)*, SIAM Journal on Discrete Mathematics 32 (2018), 2381--2397.

---

## Appendix A. Epistemic ledger for v0.32
...
| weighted frontier-DAG mass identity | **PROVED IN SANDBOX + TOY FALSIFICATION CHECKS; CLEAN-ROOM AUDIT PENDING** |
| survival-area identity \(A_{\rm surv}=E[T]\) | **PROVED IN SANDBOX** |
| quotient-DAG UNSAT certificate | **PROVED SOUND IN SANDBOX; CLEAN-ROOM AUDIT PENDING** |
...
## Appendix B. Editorial changes through v0.32
