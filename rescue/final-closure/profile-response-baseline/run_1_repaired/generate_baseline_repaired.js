...
fs.writeFileSync('scratch/profile-response-baseline-h2-h7-2026-08-25/run_1_repaired/PROFILE_BASELINE.json', JSON.stringify(baselineOut, null, 2));
fs.writeFileSync('scratch/profile-response-baseline-h2-h7-2026-08-25/run_1_repaired/SUMMARY.json', JSON.stringify(sumOut, null, 2));

let csv = Object.keys(baselineOut[0]).join(',') + '\n';
for(let rec of baselineOut) {
  csv += Object.values(rec).join(',') + '\n';
}
fs.writeFileSync('scratch/profile-response-baseline-h2-h7-2026-08-25/run_1_repaired/PROFILE_BASELINE.csv', csv);
