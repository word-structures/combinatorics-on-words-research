...
**Working manuscript v0.27 — 2026-08-27**
...
second full population implementation.
...
fails the final finite gate.  A newer no-C local search, seeded from a
D-active exact scaffold family, reduces the 12 actual no-C trigram violation
count from 44 to
\[
\boxed{31}.
\]
This is only a heuristic near-miss: exact exhaustive extension of the
surrounding recorded AF populations still gives \(0ABDEF\).  The first exact
ABDEF scaffold therefore remains the immediate positive-search target.
...
\(E,B,D\)-extension search.  The successive exact set unions were

\[
9693\to15995\to16995\to17444\to17519\to17587
\to18672\to20672\to20786.
\]

The last step combines the 20672-word union with a 14-state D-active
neighborhood and a disjoint 100-word boundary-preserving population.  Exact
set intersections are zero and the canonical union check gives
...
Thus 20786 is recorded as an exact lower bound, but not yet as a
two-implementation population theorem.
...
For every new internally clean F word outside the current union:

1. enumerate the **complete** A-profile satisfying
   \[
   A,\ AF,\ FA,\ FAF;
   \]
2. for every exact AF pair, search E and B under the actual no-C factor
   language;
3. solve D under all actual D-containing contexts simultaneously;
4. if and only if an exact ABDEF scaffold exists, enumerate C under all
   remaining actual \(h_6\) bigram/trigram contexts;
5. send the first complete \(ABCDEF=H\) immediately to the independent
   22-trigram finite gate;
6. only a finite-gate PASS proceeds to Gate T.

The no-C near-31 local-search record is useful only as a guide to promising
basins.  It is not a certificate and cannot prune a negative branch.
