...
**Working manuscript v0.23 — 2026-08-27**
...
\]
On the F-role side, a 279-state internally clean transposition component has been globally excluded over the complete A-profile space:
\[
279F\to24\text{ AF-positive }F
\to470\text{ AF-A}
\to971\text{ AFD-D}
...
\to0\text{ ABCDF}.
\]
The unique bridge-bearing F family has an independently reproduced 325-record bridge set and an independent Python C replay.  Together with earlier fixed-F certificates, at least 320 distinct length-40 F-role blocks are globally excluded as \(H(f)\).  For periods above 40, a fail-closed Rao--Rosenfeld parent/ancestor certifier has passed both a negative \(g_3\) regression and a positive-witness control.  The complete six-block coding remains open.
...
### 8.28 Conservative global F-role exclusion ledger
...
Therefore the current conservative union contains

\[
\boxed{320}
\]

distinct length-40 F-role blocks that are globally impossible as \(H(f)\).

This is a lower bound on excluded F words, not a classification of the entire
F-role profile space.
...
The current exact data identify the reciprocal B/C seam as the next rare
obstruction.  A D-bridge is necessary but not sufficient: in the first
bridge-bearing fixed-F family, 325 exact D-bridge records produced 48 complete
C words under \(AC,BC,DC\), and all 48 failed \(CB\).

The preferred discovery/certification architecture is now

\[
\boxed{
F
\to
\text{global AF family}
\to
\text{all AFD-compatible D}
\to
\text{D support from C and B}
\to
\text{reciprocal }BC/CB
\to
ABCDF
\to
E.
...
1. discover AF-rich internally clean F-role seeds rather than uniformly random
   F words;
2. enumerate the full F-swap component around a promising seed;
3. for each F, enumerate the complete A-profile family satisfying
   \(AF,FA,FAF\);
4. enumerate every AFD-compatible D;
5. require simultaneous C-support (\(AC,DC\)) and B-support
   (\(FB,BDF,DFB,FBD\));
6. for each D-bridge, enforce \(BC,CB,CBC,BDC,CBD,DCB\);
7. only an ABCDF core pays the E-search cost;
8. run the complete 22-trigram period-\(2,\ldots,40\) certifier;
9. send any finite-gate-zero H immediately to the fail-closed \(K>40\)
   Rao--Rosenfeld parent/ancestor certificate.

Every negative statement must identify the exact finite state space exhausted.
A stronger pruning predicate is forbidden unless it is an actual
\(h_6\)-factor constraint or is separately proved implied by those constraints.
