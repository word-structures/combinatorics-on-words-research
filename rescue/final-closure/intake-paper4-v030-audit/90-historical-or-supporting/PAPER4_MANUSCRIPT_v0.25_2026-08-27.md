...
**Working manuscript v0.25 — 2026-08-27**
...
\]
Subsequent complete F-component closures raise the conservative exact union to
\[
\boxed{5151}
\]
distinct length-40 F-role blocks globally excluded as \(H(f)\).

At the five-role level, the current recorded exact AF population contains 702
distinct \((A,F)\) pairs satisfying \(AF,FA,FAF\).  A two-orientation exhaustive
extension search proves
\[
\boxed{702\text{ recorded AF pairs}\to0\text{ ABDEF scaffolds}}
\]
under every actual \(h_6\) bigram/trigram condition not involving C.

The positive search remains open.  Its current independently recomputed
internally-clean six-role search record has 146 period-\(2,\ldots,40\)
violating windows across the 22 actual trigrams; it is not a candidate proof
and the fail-closed final certifier rejects it at `ace`, start 37,
half-period 2, factor `baba`.
...
### 8.32 Conservative global F-role ledger: 5151 words
...
The negative certificate program and the positive construction program now
have different roles.

### 9.1 Negative certification

Component and fixed-role searches continue to produce exact lower bounds and
structural obstructions.  They should be expanded only when they add a
complete component, a stronger theorem, or a reusable obstruction.  The
current conservative F-role lower bound is 5151 globally excluded words, and
the recorded 702-word AF population is closed at ABDEF.

### 9.2 Positive synthesis

The principal priority is now a genuinely new AF family outside the recorded
702 pairs.

The strongest observed obstruction is E support.  Consequently the preferred
direct construction order is
...
1. construct E under \(FE\);
2. construct A simultaneously under \(FA,EA,FEA\), then require
   \(AF,AFE,EAF,FAF\);
3. construct B simultaneously under \(FB,EB\);
4. construct D under every actual no-C context
   \[
   AD,BD,DF,ADF,BDF,DFA,DFB,EAD,EBD,FAD,FBD;
   \]
5. only an ABDEF scaffold pays the C-search cost;
6. C must satisfy all remaining actual bigram/trigram contexts;
7. the complete H is sent immediately to the one-command finite gate and,
   only if finite-zero, to Gate T.

A 300-F discovery sweep outside the currently excluded F union has not yet
produced an ABDEF scaffold.  Some seeds hit node caps, so this is a search
observation and not a finite nonexistence certificate.

### 9.3 Final certification

Any proposed H must pass, in order,

\[
\boxed{
\text{role profiles/kernel}
\to
22\text{-trigram }p=2,\ldots,40
\to
\text{Gate T source realizability}.
}
\]

No search-engine score, incremental zero, or partial scaffold is a theorem
until the independent fail-closed certifier passes.

The global problem remains open.
