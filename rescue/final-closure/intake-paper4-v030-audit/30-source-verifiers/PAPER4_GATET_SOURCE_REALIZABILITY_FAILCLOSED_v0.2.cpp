...
int main(int argc,char**argv){
 ...
 // Exact initial-target expanding bounds from the actual outer-parent list.
 // We store integer ceilings, tested with exact quadratic-irrational comparisons.
 auto ceilAbsAlg=[](long long A,long long B){
   for(int k=0;;k++) if(absAlgLE(A,B,k,0)) return k;
 };
 int initB3=0,initBp=0,initBm=0;
 long double targetMaxPlus=0,targetMaxMinus=0;
 for(auto&t:initial){
  long long S=0,A=0,B=0;
  for(int i=0;i<6;i++){S+=t.d[i];A+=(long long)RA[i]*t.d[i];B+=(long long)RB[i]*t.d[i];}
  initB3=max(initB3,(int)llabs(S));
  initBp=max(initBp,ceilAbsAlg(A,B));
  initBm=max(initBm,ceilAbsAlg(A,-B));
  targetMaxPlus=max(targetMaxPlus,fabsl((long double)A+sqrtl(3.0L)*(long double)B));
  targetMaxMinus=max(targetMaxMinus,fabsl((long double)A-sqrtl(3.0L)*(long double)B));
 }
 cout<<"TARGET_MAX_R3 "<<initB3<<"\n";
 cout<<"TARGET_CEIL_RPLUS "<<initBp<<"\n";
 cout<<"TARGET_CEIL_RMINUS "<<initBm<<"\n";
 cout<<"TARGET_MAX_RPLUS_NUMERIC "<<fixed<<setprecision(6)<<(double)targetMaxPlus<<"\n";
 cout<<"TARGET_MAX_RMINUS_NUMERIC "<<fixed<<setprecision(6)<<(double)targetMaxMinus<<"\n";

 // Exact h6 parent-correction maxima imply invariant integer thresholds:
 //
 // lambda=3:        C3=6              => 6/(3-1)=3.
 // lambda=+sqrt(3): C+=18+9sqrt(3)   => C+/(sqrt(3)-1)
 //                                      =(45+27sqrt(3))/2 < 46.
 // lambda=-sqrt(3): C-=-1+4sqrt(3) in absolute value
 //                    => C-/(sqrt(3)-1)=(11+3sqrt(3))/2 < 9.
 //
 // Therefore any ancestor chain beginning in the input list satisfies the
 // expanding bounds below.
 int B3=max(initB3,3);
 int Bp=max(initBp,46);
 int Bm=max(initBm,9);
 cout<<"ANCESTOR_BOUND_R3 "<<B3<<"\n";
 cout<<"ANCESTOR_BOUND_RPLUS "<<Bp<<"\n";
 cout<<"ANCESTOR_BOUND_RMINUS "<<Bm<<"\n";
 // ------------------------------------------------------------
 // 2. Global sound ancestor vector box.
 //
 // Contracting realizable factor-difference bounds: |Qd| <= (4,4,2).
 //
 // To enumerate every integer d satisfying these Q-bounds and the dynamic
 // expanding bounds, use the exact inverse of
 // T=[Q;r3;r+;r-]. Every Q coefficient in T^{-1} has absolute value <=1/3,
 // the r3 coefficient is 1/6, and every r+/r- coefficient has absolute
 // value <1/2 (use sqrt(3)<9/5 on the explicit inverse).
 //
 // Hence every coordinate satisfies
 //
 // |d_i| < 10/3 + B3/6 + (Bp+Bm)/2.
 //
 // Let N be the ceiling of this rational upper bound. Enumerating the
 // free variables d4,d5,d6 in [-N,N] is therefore sound; exact algebraic
 // predicates below discard points outside the true box.
 int N=(20+B3+3*(Bp+Bm)+5)/6; // ceil((20+B3+3(Bp+Bm))/6)
 cout<<"FREE_COORD_BOUND "<<N<<"\n";
 vector<V6> box;
 box.reserve(100000);
 ...
 for(int F=-N;F<=N;F++){
  ...
  if(abs(sum)>B3)continue;
  if(!absAlgLE(aa,bb,Bp,0))continue;
  if(!absAlgLE(aa,-bb,Bm,0))continue;
  box.push_back(d);
 }
 ...
 cout<<"STATUS "<<(hit?"REALIZABLE_OUTER_PARENT_FOUND":"ALL_OUTER_PARENTS_NONREALIZABLE")<<"\n";
 return hit?2:0;
}
