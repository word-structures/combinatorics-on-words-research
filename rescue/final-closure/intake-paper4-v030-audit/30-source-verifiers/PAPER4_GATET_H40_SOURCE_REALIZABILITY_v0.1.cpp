...
int main(int argc,char**argv){
 ...
 // Universal H40 outer-parent envelope.
 // For prescribed M40, |v_j|<=80 and |Qd|<=(4,4,2).  Exact inversion of
 // [M40;Q] gives |r3(d)|<=6, |r+(d)|<156, |r-(d)|<25.
 // Runtime checks make the envelope fail-closed.
 long long targetMaxSum=0; long double targetMaxPlus=0,targetMaxMinus=0;
 for(auto&t:initial){
  long long S=0,A=0,B=0;
  for(int i=0;i<6;i++){S+=t.d[i];A+=(long long)RA[i]*t.d[i];B+=(long long)RB[i]*t.d[i];}
  targetMaxSum=max(targetMaxSum,llabs(S));
  ...
  if(abs(S)>6 || !absAlgLE(A,B,156,0) || !absAlgLE(A,-B,25,0))
   throw runtime_error("outer-parent target exceeds universal H40 envelope");
 }
 cout<<"TARGET_MAX_R3 "<<targetMaxSum<<"\n";
 cout<<"TARGET_MAX_RPLUS "<<fixed<<setprecision(6)<<(double)targetMaxPlus<<"\n";
 cout<<"TARGET_MAX_RMINUS "<<fixed<<setprecision(6)<<(double)targetMaxMinus<<"\n";
 ...
 //
 // Universal H40 initial-target bounds are (6,156,25) on r3,r+,r-.
 // The h6 parent-correction invariant thresholds (3,30+18√3,6+2√3)
 // are smaller, so (6,156,25) is invariant under repeated parenthood.
 //
 // Exact inversion of T=[Q;r3;r+;r-], with 5/3<√3<7/4, bounds every
 // integer coordinate by <20.  We enumerate the deliberately looser
 // free-variable cube [-21,21]^3 and accept only exact algebraic members.
 // ------------------------------------------------------------
 vector<V6> box;
 box.reserve(100000);
 ...
 for(int F=-21;F<=21;F++){
  ...
  if(abs(sum)>6)continue;
  if(!absAlgLE(aa,bb,156,0))continue;
  if(!absAlgLE(aa,-bb,25,0))continue;
  box.push_back(d);
 }
 ...
 cout<<"STATUS "<<(hit?"H40_REALIZABLE_OUTER_PARENT_FOUND":"H40_OUTER_PARENTS_ALL_NONREALIZABLE")<<"\n";
 return hit?2:0;
}
