...
## Global fixed-F update R4

New exact claim:

\[
41\text{ distinct F-role blocks}
\to
465\text{ family-counted AF-A}
\to
976\text{ AFD-D}
\to
...
Interpretation:
each of the 41 concrete F blocks is globally excluded as \(H(f)\), regardless
of the A-swap component.

Claude must verify that the fixed-F proof inserts the true
\(H(a),H(d),H(c),H(b)\) into exhaustive sets and that every support predicate
is an actual \(h_6\) factor.


## Global F-component update R5

New canonical claims:

### C23 — 279-state F component
\[
|C_F|=279,\quad
279F\to24\text{ AF-positive F}\to470AF\text{-A}\to971D
\to757C\text{-support}\to325\text{ bridges}\to0ABCDF.
\]
...
- Python and C++ F-component BFS agree exactly;
- identical component SHA256;
- the unique bridge-bearing F96 family has full independent replay.

### C24 — F96 full replay
\[
50A\to371D\to344C\text{-support}\to325\text{ bridges}
\to48C\to0CB\to0ABCDF.
\]

A, D, C-support and bridge **record sets**, not only counts, agree between
primary and independent implementations.

Verdict target:
`PASS / NEEDS_NO_FURTHER_REPLAY`.

### C25 — conservative global F ledger
\[
320
\]
distinct role-F words are globally excluded as \(H(f)\).

This is a lower bound and must never be described as all possible F words.

Verdict target:
`PASS`.

### C26 — problem freshness
No 2024--2026 source found in the targeted 2026-08-27 search claims a solution
of the exact Mäkelä ternary conjecture.

Required wording:
`NO SOLUTION FOUND IN SOURCES SEARCHED`.

Verdict:
`NEEDS_DATABASE_AUDIT` for strong openness/priority wording.
