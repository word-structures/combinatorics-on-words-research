...
## 27. Route-C Uniform L=6 family closed (S_small(6) n S_large(6) = empty)

*Logged 2026-08-16. See MATH_CLAIMS.md row 112.*

**Hypothesis:** A uniform L=6 ternary coding g of the h6 morphism could yield an infinite aa2f word, resolving Mäkelä's conjecture via Route C.

**Why it was shot down:**
- The uniform L=6 coding family consists of exactly 1,200,636 concrete codings (200,106 canonical classes) satisfying the small-window K in [2,5] avoidance condition.
- A block-aligned kernel elimination (Stage-A) using Parikh matrix properties removes 1,200,288 of these codings by exposing finite structural squares of K >= 6.
- The remaining residue class of 348 codings was independently and exhaustively confirmed by a dual-checker audit to contain explicit Abelian squares with K in [6,10] ending by position 34.
- 0 out of 1,200,636 candidates survive.
- This proves S_small(6) n S_large(6) = empty.

**Conclusion:**
- No uniform L=6 coding of h6 can yield an aa2f word.
- Unlike H4/L5, this closure spans the exact combined set within extremely bounded limits, requiring no exhaustive deep search of large unpruned populations.
- **Explicit boundary:** This does NOT prove S_large(6) = empty, does not alter L*, does not extend to L >= 7 or non-uniform codings, and does not solve Mäkelä's conjecture.
- **Finality: BOUNDED COMPUTED.** Exhaustive, exact proof for uniform L=6. Evidence capsule archived at docs/research/evidence/l6-route-c-closure-2026-08/.
