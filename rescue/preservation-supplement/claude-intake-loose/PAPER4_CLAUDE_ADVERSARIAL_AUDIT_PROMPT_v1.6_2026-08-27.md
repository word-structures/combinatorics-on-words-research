...
**Version 1.6 — canonical v0.23 audit with 279-F / F96 full replay**
...
## Mandatory fixed-F global audit

Audit `PAPER4_GLOBAL_F_EXCLUSION_41F_v1.0_2026-08-27.md`.

Attack the theorem in this order:

1. For fixed F, does the A enumerator cover the complete role-A profile space?
2. Do `AF,FA,FAF` include every necessary A/F context used at this stage?
3. Is every AFD-compatible D enumerated?
4. Is C-support through `AC,DC` genuinely necessary for every full coding?
5. Is B-support through `FB,BDF,DFB,FBD` genuinely necessary?
6. Does emptiness of the intersection really exclude F globally,
   independently of A-swap components?
7. Verify aggregate arithmetic:
   41 F, 465 AF-A, 976 AFD-D, 468 C-support D, 0 bridges.
8. Check that the manuscript never upgrades this to exclusion of all F words.

A counterexample to the fixed-F theorem is manuscript-blocking.


## Mandatory audit target — 279-state F component

Audit:
`PAPER4_F_COMPONENT_GLOBAL_EXCLUSION_279_v1.1_2026-08-27.md`.

Required attacks:

1. independently verify the F transposition component definition;
2. verify the 279-state component SHA;
3. verify that 255 F words are AF-empty and 24 are AF-positive in the recorded
   exhaustive sweep;
4. verify the aggregate
   \[
   470AF\text{-A}\to971D\to757C\text{-support};
   \]
5. verify that every fixed-F gate is necessary for a hypothetical full H;
6. audit the unique F96 family separately;
7. verify that 325 bridge records are complete;
8. verify that all 325 die at the full C gate;
9. reject any inference from one F component to the entire F-profile space.

## Mandatory F96 record-set replay audit

Audit:
`PAPER4_F96_FULL_INDEPENDENT_REPLAY_CERTIFICATE_v1.0_2026-08-27.md`.

Do not accept matching counts alone. Verify the saved comparisons show exact
record-set equality for:
- 50 A words;
- 371 AFD-D records;
- 344 C-support pairs;
- 325 D-bridge records.

Then inspect the independent Python C replay:
\[
325\to48\to0CB\to0ABCDF.
\]

## Mandatory manuscript consistency audit

Canonical manuscript:
`PAPER4_MANUSCRIPT_v0.23_2026-08-27.md`.

Verify that:
- §8.11 is historical, not a current 18-component claim;
- C5/C8 remain pending and excluded from safe ledgers;
- §9 uses the current D-bridge / reciprocal BC-CB architecture;
- Gate T is not called OPEN after §8.14;
- 320 F is explicitly a lower bound;
- no sentence claims Mäkelä solved;
- novelty remains `NOT_ESTABLISHED` / `NOVELTY_UNRESOLVED`.

## Mandatory literature freshness audit

Use `PAPER4_LITERATURE_AUDIT_v0.7_2026-08-27.md`.

Pay special attention to:
- Mathematics of Computation DOI 10.1090/mcom/4246;
- Currie--Mol--Rampersad--Shallit 2024;
- Filimonova--Puzynina 2026;
- absence of any checked 2024--2026 source claiming the exact Mäkelä solution.

Return `NOVELTY_UNRESOLVED` unless independently established.
