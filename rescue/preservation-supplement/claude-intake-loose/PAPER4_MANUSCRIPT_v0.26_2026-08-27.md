...
**Working manuscript v0.26 — 2026-08-27**
...
population-level independent replay raise the conservative exact union to
\[
\boxed{9693}
\]
...
The current generated F pool is exhausted: all 4544 internally clean F words
present in the pool are contained in the exact 9693-word exclusion union.

### 9.1 Negative certification

Further negative computation is useful only when it expands the canonical
union with genuinely new F words or gives a reusable structural obstruction.
The current conservative exact lower bound is

\[
\boxed{9693}
\]

globally excluded F-role words.

The 4242-F direct-A population has a full independent population replay, and
its 19 AF-positive words are closed independently at ABDEF in both E
orientations.

### 9.2 Positive synthesis

The principal priority is therefore **new F generation outside the 9693
union**.

For every fresh internally clean F word:

1. run the exact fixed-F A solver for
   \[
   A,\ AF,\ FA,\ FAF;
   \]
2. if A exists, enumerate **all** compatible A words for that F rather than
   stopping at the first witness;
3. close the resulting AF family under the two-orientation no-C search
   \[
   AF\to E\to B\to D;
   \]
4. only an ABDEF scaffold pays the C-search cost;
5. C must satisfy all remaining actual \(h_6\) bigram/trigram contexts;
6. the first complete ABCDEF coding is sent immediately to the independent
   finite gate and then Gate T.

The current direct complete-H search record of 146 finite violations remains
a useful heuristic reference, but it is not a theorem and it is no longer the
main search architecture.

### 9.3 Final certification

Any proposed H must pass
...
No search score, local-search zero, or partial scaffold is a result until the
independent fail-closed certifier passes.
