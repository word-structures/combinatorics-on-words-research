...
**Working manuscript v0.29 — 2026-08-27**
...
A further factorization of the no-C language isolates the \(A,E,F\) subproblem.
A dedicated search finds an internally clean prescribed-profile \(AEF\)
record with exactly
\[
\boxed{2}
\]
remaining violating windows, both in the actual trigram \(FEA\).  Exhaustive
two-swap neighborhood enumeration again finds no improvement.  These are
search diagnostics only: no exact \(AEF\), \(ABDEF\), or complete \(H\) is
known.  The first exact \(ABDEF\) scaffold remains the breakthrough trigger
...
This identifies A/E/F compatibility as a stronger positive-search bottleneck
than the earlier D-prefix diagnostics suggested.
...
**Status:** `EXACT-CHECKED SEARCH RECORD / AEF OPEN`.
...
**Status:** `EXACT-CHECKED LOCAL RIGIDITY / GLOBAL AEF OPEN`.
...
The search now uses two nested milestones.

The first positive subproblem is the exact A/E/F core:

\[
\boxed{AEF}.
\]

The current search record has two residual \(FEA\) violations and is an exact
two-swap local minimum.  Progress on this subproblem should therefore come
from higher-order profile-preserving cycles, global SAT/MILP/CP-style
constraint solving, or a new structural reduction.

The construction pipeline is:

1. produce an exact internally clean prescribed-profile \(AEF\);
2. add B under the actual \(FB,EB\) contexts;
3. solve D under all actual D-containing contexts;
4. verify the resulting five-role scaffold against the complete no-C factor
   language;
5. if and only if an exact \(ABDEF\) scaffold exists, enumerate C under all
   remaining actual \(h_6\) bigram/trigram contexts;
6. send the first complete \(ABCDEF=H\) immediately to the independent
   22-trigram finite gate;
7. only a finite-gate PASS proceeds to Gate T.

The A/B factorization of §8.41 may also be used in the reverse direction:
for promising fixed \(D,E,F\) cores, search A and B independently and combine
them only when both existential role problems succeed.

The no-C near-31 and AEF best-2 records are search guides only.  Neither may
be used as a negative certificate.
