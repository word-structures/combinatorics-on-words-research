...
aedge=Q['aedge']
Aa0=sp.csr_matrix((mult*((tar==0)&(aedge==1)),(rr,cc)),shape=(K,K));Ba=sp.csr_matrix((mult*((tar==1)&(aedge==1)),(rr,cc)),shape=(K,K))
...
NA=transform(A0);NB=transform(B);NAA=transform(Aa0);NBA=transform(Ba)
...
def blocks(M):
 ...
 return TT,SS,TS,ST
Atri,Astd,Ats,Ast=blocks(NA);Btri,Bstd,Bts,Bst=blocks(NB);Aatri,Aastd,Aats,Aast=blocks(NAA);Batri,Bastd,Bats,Bast=blocks(NBA)
...
sp.save_npz(f'/mnt/data/h8cp/s3_Aatri_pid{pid}.npz',Aatri);sp.save_npz(f'/mnt/data/h8cp/s3_Aastd_pid{pid}.npz',Aastd);sp.save_npz(f'/mnt/data/h8cp/s3_Aats_pid{pid}.npz',Aats);sp.save_npz(f'/mnt/data/h8cp/s3_Aast_pid{pid}.npz',Aast)
sp.save_npz(f'/mnt/data/h8cp/s3_Batri_pid{pid}.npz',Batri);sp.save_npz(f'/mnt/data/h8cp/s3_Bastd_pid{pid}.npz',Bastd);sp.save_npz(f'/mnt/data/h8cp/s3_Bats_pid{pid}.npz',Bats);sp.save_npz(f'/mnt/data/h8cp/s3_Bast_pid{pid}.npz',Bast)
