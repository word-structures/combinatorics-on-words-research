...
np.savez_compressed(f'/mnt/data/h8cp/a_tilt_quotient_pid{pid}.npz',cls=cls,sizes=sizes,rep=rep,rows=rows,cols=cols,target=targs,aedge=acats,mult=mult,state_a=state_a,profile=profiles[pid-1])
...
open(f'/mnt/data/h8cp/a_tilt_quotient_pid{pid}.json','w').write(json.dumps(meta,indent=2));print(meta)
