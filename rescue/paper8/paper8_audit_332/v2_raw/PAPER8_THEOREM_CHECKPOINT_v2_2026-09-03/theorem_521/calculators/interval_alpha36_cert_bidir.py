import numpy as np, scipy.sparse as sp, math, json
from collections import deque
BASE=np.load('/mnt/data/p8work/a_tilt_quotient_pid4.npz')
...
def certify(a,b,m=36,batch=64,reverse=False,round_pad=2e-10):
 mid=(a+b)/2;r=pfvec(mid,reverse);Aa=matrix(a,reverse);Ab=matrix(b,reverse)
 va=r.copy();vb=r.copy()
 for _ in range(m):va=Aa@va;vb=Ab@vb
 sa=va/r;sb=vb/r
 smin=float(sa.min())*(1-round_pad);smax=float(sb.max())*(1+round_pad);kappa=smax/smin
 ...
 return dict(a=a,b=b,mid=mid,m=m,reverse=reverse,kappa=kappa,alphaQ_lower=alphaQ,R_projective_upper=R,alphaP_lower=alphaP,smin=smin,smax=smax,denom=den)
if __name__=='__main__':
 for rev in [False,True]:
  print('REV',rev)
  for a,b in [(0,.005),(0,.002),(.5,.505),(.995,1)]:
   o=certify(a,b,reverse=rev);print(a,b,o['alphaP_lower'],o['alphaQ_lower'],o['kappa'])
