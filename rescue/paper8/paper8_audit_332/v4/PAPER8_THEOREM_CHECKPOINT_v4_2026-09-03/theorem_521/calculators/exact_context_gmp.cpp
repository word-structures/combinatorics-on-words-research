...
void step(const vector<InList>& inc, const Mom& in, Mom& out, int K,int D,bool targetOnly=false){
...
int main(int argc,char**argv){
 if(argc<4){cerr<<"usage edges.bin L out.txt\n";return 2;} string fp=argv[1];int L=stoi(argv[2]);string outp=argv[3];
 ifstream f(fp,ios::binary);uint32_t K,E;f.read((char*)&K,4);f.read((char*)&E,4);vector<int32_t> rr(E),cc(E);vector<uint8_t> tt(E),aa(E);f.read((char*)rr.data(),4*E);f.read((char*)cc.data(),4*E);f.read((char*)tt.data(),E);f.read((char*)aa.data(),E);
 vector<InList> inc(K);for(uint32_t e=0;e<E;e++)inc[cc[e]].e.push_back({rr[e],tt[e],aa[e]});
 vector<int64_t> sizesv(K); { ifstream sf("/mnt/data/p8work/pid4_sizes.bin",ios::binary); sf.read((char*)sizesv.data(),8*K); }
 int n=2*L+1; // safe exact degree cap from path max computed externally; use n/4+4 generous
 int D=(n*23)/100+4; // for L180: 87 >= actual81; L150:73 >=68
 size_t SZ=(size_t)K*(D+1); cerr<<"K "<<K<<" E "<<E<<" L "<<L<<" n "<<n<<" D "<<D<<" cells "<<SZ<<"\n";
 Mom A(SZ),B(SZ),Den(SZ),Num(SZ),TmpD(SZ),TmpN(SZ);
 for(uint32_t i=0;i<K;i++) A.c[IX(i,0,D)]=sizesv[i];
 auto t0=chrono::steady_clock::now();
 for(int k=0;k<L;k++){step(inc,A,B,K,D,false); swap(A,B);}
 cerr<<"left done "<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n";
 step(inc,A,Den,K,D,false); step_target_derivative(inc,A,Num,K,D);
 for(int k=0;k<L;k++){step(inc,Den,TmpD,K,D,false);swap(Den,TmpD);step(inc,Num,TmpN,K,D,false);swap(Num,TmpN);}
 cerr<<"right done "<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n";
 ...
 vector<mpz_class> D1(D+1),N1(D+1),D2(D+1),N2(D+1);
 for(int d=0;d<=D;d++){
   D1[d]=3*Ds[d]-n*Dc[d]; N1[d]=3*Ns[d]-n*Nc[d];
   D2[d]=9*Dv[d]-6*n*Ds[d]+(mpz_class)n*n*Dc[d];
   N2[d]=9*Nv[d]-6*n*Ns[d]+(mpz_class)n*n*Nc[d];
 }
 auto A1=conv(N1,Dc), B1=conv(D1,Nc), A2=conv(N2,Dc), B2=conv(D2,Nc);
 size_t deg=max(A2.size(),B2.size());vector<mpz_class> F(deg),G(max(A1.size(),B1.size()));
 for(size_t i=0;i<deg;i++){if(i<A2.size())F[i]+=A2[i];if(i<B2.size())F[i]-=B2[i];}
 for(size_t i=0;i<G.size();i++){if(i<A1.size())G[i]+=A1[i];if(i<B1.size())G[i]-=B1[i];}
