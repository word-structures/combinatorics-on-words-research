...
PATH='/mnt/data/paper8_resume/unpacked/PAPER8_THEOREM_CHECKPOINT_v2_2026-09-03/raw_research_state/p8work/a_tilt_quotient_pid3.npz'
...
if __name__=='__main__':
 tests=[(0.2,0.211,True),(0.5,0.52,True),(0.08,0.087,True)]
 out=[]
 for t in tests:
  s=time.time();z=cert(*t);z['seconds']=time.time()-s;out.append(z);print(z,flush=True)
 json.dump({'dtype':str(np.dtype(np.longdouble)),'eps':float(np.finfo(np.longdouble).eps),'pad':1e-14,'records':out,'min_alpha':min(x['alphaP_longdouble'] for x in out),'PASS':min(x['alphaP_longdouble'] for x in out)>0.91},open('/mnt/data/paper8_resume/theorem_431/LONGDOUBLE_SELECTED_INTERVAL_RECHECK.json','w'),indent=2)
