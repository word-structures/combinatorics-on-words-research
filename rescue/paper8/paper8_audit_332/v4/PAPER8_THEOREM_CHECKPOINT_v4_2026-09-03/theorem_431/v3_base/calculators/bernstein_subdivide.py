...
out={'profile':[4,3,1],'L':L,'subdivision':m,'F_degree':len(F)-1,'G_nonzero_count':sum(x!=0 for x in G),'all_numerator_bernstein_positive':all(Fraction(int(r['F_lower_num']),int(r['F_lower_den']))>0 for r in records),'global_C_lower_float':g['C_lower_float'],'global_record':g,'records':records}
